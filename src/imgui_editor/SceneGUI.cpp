...
void SceneGUI::bindResourcesToGraph()
{
    ...
    onCameraScreenSizeChange(this->renderWindow->CameraScreenSize, this->renderWindow->CameraScreenSize);
    ...
    this->renderWindow->CameraScreenSize.OnChange.add(&SceneGUI::onCameraScreenSizeChange, this);
}

// clear all loaded scene
void SceneGUI::unloadAll()
{
    this->renderWindow->CameraScreenSize.OnChange.remove(&SceneGUI::onCameraScreenSizeChange, this);
...
void SceneGUI::onCameraScreenSizeChange(const MathCore::vec2f &value, const MathCore::vec2f &oldValue)
{

    printf("OnViewportChange %f %f\n", value.width, value.height);

    // MathCore::vec2i size = MathCore::vec2i(value.w, value.h);

    float center_x = value.width * 0.5f;
    float center_y = value.height * 0.5f;
    float margin = 32 - 9;
...
void SceneGUI::OnUpdateImpl(Platform::Time *time)
{
    ...
    MathCore::vec3f pos3D = MathCore::vec3f(this->renderWindow->MousePosRelatedToCenter_OriginBottom * this->renderWindow->windowToCameraScale, 0.0f);
