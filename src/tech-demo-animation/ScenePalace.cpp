...
void ScenePalace::loadResources()
{
    ...
    skinnedMesh->loadModelBase("resources/castle_guard/castle_guard_01.bams", resourceMap, resourceMap->defaultPBRMaterial);

    skinnedMesh->loadAnimation("idle", resourceMap, "resources/castle_guard/castle_guard_01@Idle.bams");
    skinnedMesh->loadAnimation("walk", resourceMap, "resources/castle_guard/castle_guard_01@Walking.bams");
    skinnedMesh->loadAnimation("run", resourceMap, "resources/castle_guard/castle_guard_01@Run.bams");
    ...
    newMaterial->property_bag.getProperty("roughness").set<float>(0.5f);
    newMaterial->property_bag.getProperty("metallic").set<float>(1.0f);

    newMaterial->property_bag.getProperty("texAlbedo").set<std::shared_ptr<AppKit::OpenGL::VirtualTexture>>(Texture_Guard[0]);
    newMaterial->property_bag.getProperty("texNormal").set<std::shared_ptr<AppKit::OpenGL::VirtualTexture>>(Texture_Guard[1]);
    newMaterial->property_bag.getProperty("texSpecular").set<std::shared_ptr<AppKit::OpenGL::VirtualTexture>>(Texture_Guard[2]);
