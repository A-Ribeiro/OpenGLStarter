...
namespace AppKit
{
    namespace Physics
    {
        namespace Util
        {
            ...
            //     inline static bool CheckBoxOverlap(const type &item, const MathCore::vec2f &min, const MathCore::vec2f &max) { return item.checkBoxOverlap(min, max); }
            //     inline static const MathCore::vec2f &GetBoxMin(const type &item) { return item.box.min; }
            //     inline static const MathCore::vec2f &GetBoxMax(const type &item) { return item.box.max; }
            // };
            ...
            struct QuadtreeNode
            {
                ...
                inline Quadrant computeQuadrant(const MathCore::vec2f &p) const { return QuadrantFromBitSet(p.x >= box_center.x, p.y >= box_center.y); }
                // Example of QuadtreeIntegration:
                // struct QuadtreeIntegration
                // {
                //     using type = Structure2D;
                ...
                inline void subdivide(const std::vector<typename QuadTreeIntegrationT::type> &items, int32_t initial_index_count)
                {
                ...
                inline static bool box_isPointInside(const MathCore::vec2f &point, const MathCore::vec2f &min, const MathCore::vec2f &max)
                {
                ...
                inline static bool box_overlaps(const MathCore::vec2f &min1, const MathCore::vec2f &max1, const MathCore::vec2f &min2, const MathCore::vec2f &max2)
                {
                    using namespace MathCore;
                ...
                inline bool contains(const MathCore::vec2f &p) const { return box_isPointInside(p, box.min, box.max); }
                inline bool contains(const MathCore::vec2f &min, const MathCore::vec2f &max) const { return box_overlaps(box.min, box.max, min, max); }
                inline bool contains(const QuadtreeNode &other) const { return box_overlaps(box.min, box.max, other.box.min, other.box.max); }
                inline bool contains(const Core::Box2D &other) const { return box_overlaps(box.min, box.max, other.min, other.max); }
                ...
                // };
                template <typename QuadTreeIntegrationT>
                inline bool contains(const typename QuadTreeIntegrationT::type &item) const { return QuadTreeIntegrationT::CheckBoxOverlap(item, box.min, box.max); }
                    ...
                    else
                    {
                        ...
                        for (uint32_t idx : *indices)
                        {
                            ...
                            if (box_overlaps(item_min, item_max, box.min, box.max))
                            {
                                ...
                                auto box_pt_rc = Core::Box2D::closestPointInBoxToSegment(item_min, item_max, a, b, closest_point_array);
                                if (box_pt_rc == Core::SegmentPointReturnType_One_Outside)
                                {
                                    MathCore::vec2f closest_point = Core::Segment2D::closestPointToSegment(closest_point_array[0], a, b);
                                    if (MathCore::OP<MathCore::vec2f>::sqrDistance(closest_point, closest_point_array[0]) <= radius_squared)
                                        result.push_back(idx);
                                }
                                else if (box_pt_rc & Core::SegmentPointReturnType_AnyIntersectOrInside_Bit)
                                    result.push_back(idx);
                            }
                        }
                    }
            ...
            public:
                ...
                Quadtree(std::vector<typename QuadTreeIntegrationT::type> *items,
                         ...
                         const Core::Box2D &initial_box = Core::Box2D()) : items(items)
                {
                    maxDepth = maxDepth_;
                    ...
                    else
                    {
                        ...
                        MathCore::vec2f _max = initial_box.max;
                        ...
                        // increase max until max could be inside the quadtree
                        int max_tries = 10; // safety to avoid infinite loop, should not be needed
                        while (!QuadtreeNode::box_isPointInside(initial_box.max, _min, _max) && max_tries-- > 0)
                            _max += size_relative;
                        if (max_tries <= 0)
                        {
                            _max = MathCore::OP<MathCore::vec2f>::lerp(_min, _max, 1.0f + MathCore::EPSILON<float>::low_precision);
                    ...
                    std::vector<uint32_t> *tmp_array)
                {
                    ...
                    Core::Box2D query_box = Core::Box2D(a, b).expand(MathCore::vec2f(radius));
