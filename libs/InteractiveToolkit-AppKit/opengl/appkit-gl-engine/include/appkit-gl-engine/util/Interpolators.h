...
namespace AppKit
{
    namespace GLEngine
    {
        ...
        class SlerpInterpolator
        {
            ...
            int lastIndexA;
            int lastIndexB;
            MathCore::quatf lastReturned;

        public:
            std::vector<Key<MathCore::quatf>> keys;
            ...
            SlerpInterpolator()
            {
                ...
                lastIndexA = 0;
                lastIndexB = 1;
                lastReturned = MathCore::quatf();
            }

            void addKey(const Key<MathCore::quatf> &v)
            {
                keys.push_back(v);
                if (keys.size() == 1)
                {
            ...
            // binary search
            MathCore::quatf getValue(float t)
            {
                if (keys.size() == 0)
                    return MathCore::quatf();
                else if (keys.size() == 1)
                ...
                lastReturned = MathCore::OP<MathCore::quatf>::slerp(keys[lastIndexA].value, keys[lastIndexB].value, lrp);

                return lastReturned;
            }

            MathCore::quatf getValue_WalkStep(float t)
            {
                if (keys.size() == 0)
                    return MathCore::quatf();
                else if (keys.size() == 1)
            ...
            MathCore::quatf getValue_slow(float t) const
            {
                ...
                return MathCore::OP<MathCore::quatf>::slerp(keys[indexA].value, keys[indexB].value, lrp);
            }
        };
    }
