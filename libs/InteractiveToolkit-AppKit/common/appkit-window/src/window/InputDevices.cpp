...
namespace AppKit
{
    ...
    namespace Window
    {
        ...
        namespace Devices
        {
            ...
            bool Joystick::sync_all_joystick_data()
            {
#if defined(_WIN32)
            ...
            bool *Keyboard::window_key_states = nullptr;
            bool Keyboard::isPressed(KeyCode code)
            {
                if (!window_key_states || (int)code >= (int)KeyCode::Count) // KeyCode::Unknown is 255
                    return false;
                return window_key_states[(int)code];
            }
            void Keyboard::setWindow_key_states(bool *array)
            {
                if (window_key_states != array)
                    window_key_states = array;
            }
            // bool Keyboard::sync_all_keyboard_data()
            // {
            //     for (int i = 0; i < (int)KeyCode::Count; i++)
            //         ___key_state[i] = sf::Keyboard::isKeyPressed(key_mapping[i]);
            //     return true;
            // }
