...
#include <mini-gl-engine/ComponentCameraOrthographic.h>
...
namespace GLEngine {
    ...
    void ParticleSystemRenderer::drawDebugPoints( 
        ...
        size *= 0.5f;
        aRibeiro::vec3 xneg = aRibeiro::vec3(-size,0,0);
        aRibeiro::vec3 xpos = aRibeiro::vec3(size,0,0);

        aRibeiro::vec3 yneg = aRibeiro::vec3(0,-size,0);
        aRibeiro::vec3 ypos = aRibeiro::vec3(0,size,0);
        
        aRibeiro::vec3 zneg = aRibeiro::vec3(0,0,-size);
        aRibeiro::vec3 zpos = aRibeiro::vec3(0,0,size);
        ...
        debugLinesShader.setColor(aRibeiro::vec4(0.0f,1.0f,0.0f,1.0f));
        debugLinesShader.setMVP(camera->viewProjection);

        OPENGL_CMD(glEnableVertexAttribArray(debugLinesShader_AttribLocation_Pos));
        OPENGL_CMD(glVertexAttribPointer(debugLinesShader_AttribLocation_Pos, 3, GL_FLOAT, false, sizeof(aRibeiro::vec3), &lines[0]));
    ...
    void ParticleSystemRenderer::draw(const Components::ComponentCamera* camera,
        ...
        for(int i=0;i<particleSystem->particles.size();i++){
            ...
            aux.uv = aRibeiro::vec2(0,0);
            normalVertex.push_back(aux);
            aux.uv = aRibeiro::vec2(0,1);
            normalVertex.push_back(aux);
            aux.uv = aRibeiro::vec2(1,1);
            normalVertex.push_back(aux);
...
*/
    ...
    void ParticleSystemRenderer::drawSoftDepthComponent24(const Components::ComponentCamera* camera,
        ...
        sf::Vector2i s = GLEngine::Engine::Instance()->app->WindowSize.value;
        aRibeiro::vec2 ss = aRibeiro::vec2(s.x,s.y);
        ...
        softShader.setScreenSize(aRibeiro::vec2( texDims[0], texDims[1] ));
        */
        ...
        if (camera->compareType(Components::ComponentCameraPerspective::Type)) {
            ...
            softShader.setCameraNearFar( aRibeiro::vec2(perspect->nearPlane, perspect->farPlane) );
        }
            ...
            softShader.setCameraNearFar( aRibeiro::vec2(ortho->nearPlane, ortho->farPlane) );
        }
