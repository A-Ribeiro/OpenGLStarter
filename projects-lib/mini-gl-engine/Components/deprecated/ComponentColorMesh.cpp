...
namespace GLEngine {
    ...
    namespace Components {
        ...
        void setTriangle(std::vector<unsigned short> *indices,
                        ...
                        aligned_vector<vec3> *uv,
                        aligned_vector<vec3> *normals,
                        aligned_vector<vec3> *tangents,
                        aligned_vector<vec3> *binormals) {
            vertices->clear();
            ...
            if (uv != NULL){
                uv->clear();
                uv->push_back(vec3(vec2(0.0f,0.0f),0));
                uv->push_back(vec3(vec2(1.0f,0.0f),0));
                uv->push_back(vec3(vec2(0.5f,0.5f),0));
            }
        ...
        void setPlane(bool _xy,
            ...
            vec3 dimension = vec3(_width, 0, _height);
            vec3 halfDim = dimension * 0.5f;
            
            if (_xy){
                vertices->clear();
                vertices->push_back(vec3(halfDim.x, halfDim.z, 0.0f));
                vertices->push_back(vec3(halfDim.x, -halfDim.z, 0.0f));
                vertices->push_back(vec3(-halfDim.x, -halfDim.z, 0.0f));
                vertices->push_back(vec3(-halfDim.x, halfDim.z, 0.0f));
            }
            ...
            if (uv != NULL){
                uv->clear();
                uv->push_back(vec3(vec2(1.0f,0.0f),0));
                uv->push_back(vec3(vec2(1.0f,1.0f),0));
                uv->push_back(vec3(vec2(0.0f,1.0f),0));
                uv->push_back(vec3(vec2(0.0f,0.0f),0));
            }
        ...
        void setBox(const vec3 &dimension,
            ...
            if (uv != NULL) {
                ...
                uv->push_back(vec3(0, 1, 0));
                uv->push_back(vec3(1, 1, 0));
                uv->push_back(vec3(1, 0, 0));
                uv->push_back(vec3(0, 0, 0));
                //
                // BOTTOM
                //
                uv->push_back(vec3(0, 1, 0));
