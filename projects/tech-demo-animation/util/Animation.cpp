...
namespace GLEngine {
    ...
    bool AnimationClip::findRootNode(Transform *t, const void* userData) {
        ...
        for (int i = 0; i < animation.channels.size(); i++) {
            const model::NodeAnimation &node_anim = animation.channels[i];
    ...
    AnimationClip::AnimationClip(const std::string &clip_name, Transform *root, const model::Animation &animation  ) {
        
        base_model = root;
        root_node = NULL;

        root->traversePreOrder_DepthFirst( TransformTraverseMethod_const(this, &AnimationClip::findRootNode), &animation);
            ...
            if (t_node != NULL){
                ...
                if (node_anim.positionKeys.size() > 0)
                    for (int j=0;j<node_anim.positionKeys.size();j++){
                        const model::Vec3Key &key = node_anim.positionKeys[j];
                        result.position.addKey( Key<aRibeiro::vec3>(key.time / animation.ticksPerSecond, key.value) );
                    }
                
                if (node_anim.scalingKeys.size() > 0)
                    for (int j=0;j<node_anim.scalingKeys.size();j++){
                        const model::Vec3Key &key = node_anim.scalingKeys[j];
                        result.scale.addKey( Key<aRibeiro::vec3>(key.time / animation.ticksPerSecond, key.value) );
                    }
                
                if (node_anim.rotationKeys.size() > 0)
                    for (int j=0;j<node_anim.rotationKeys.size();j++){
                        const model::QuatKey &key = node_anim.rotationKeys[j];
                        result.rotation.addKey( Key<aRibeiro::quat>(key.time / animation.ticksPerSecond, key.value) );
                    }
        ...
        ARIBEIRO_ABORT(root_node_animation_index == -1, "No bone root animation found...");
    ...
    void AnimationMixer::addClip(AnimationClip* clip) {
        ...
        ARIBEIRO_ABORT(transitions.size() > 0,"trying to add clip after the animation transition computation.\n");
    ...
    void AnimationMixer::computeTransitions() {
        ARIBEIRO_ABORT(transitions.size() > 0,"trying to initialize the animation twice.\n");

        //normalize the scale of all clips
        

        Transform *model_from_file = clips_array[0]->base_model->getChildAt(0);
        aRibeiro::vec3 scale = model_from_file->getScale();
        ...
        if (current_clip == NULL) {
            ...
            if (rootMotionAnalyser.method != NULL && rootMotionAnalyser.data.dirty) {
        ...
        for (int i = 1; i < clips_array.size(); i++) {
            ARIBEIRO_ABORT(clips_array[i]->root_node != current_clip->root_node, "All animations need to share the root node with an animation attached to it.\n");
        }
        ...
        for(int index_a=0;index_a<clips_array.size();index_a++){
            for(int index_b=0;index_b<clips_array.size();index_b++){
                ...
                ARIBEIRO_ABORT( transition.matching_a_elements.size() != transition.matching_b_elements.size(), "Error on compute animation transition matrix.\n" );
    ...
    void AnimationMixer::update(float elapsed_secs) {
        if (request_queue.size() == 0) {
            ...
            if (anim_target->reachedEnd()) {
                ...
                ARIBEIRO_ABORT(matching_a_elements->size() != matching_b_elements->size(), "Matching elements different sizes.\n");
                ...
                for (int i = 0; i < matching_a_elements->size(); i++) {
                    ...
                    ARIBEIRO_ABORT(node_a->node != node_b->node, "Node A transform different from node B transform.\n");
    ...
    void RootMotionAnalyserData::setFromNodeAnimation( NodeAnimation *node_animation, float time, float weight ){
        ...
        aRibeiro::vec3 target_local_position = node_animation->samplePos(time, &_clipInfo->position_delta_interframe);
    ...
    void RootMotionAnalyserData::setFromNodeAnimationBlend( 
        ...
        aRibeiro::vec3 target_local_position_a = node_animation_a->samplePos(time_a, &clipInfo_a->position_delta_interframe);
        aRibeiro::vec3 target_local_position_b = node_animation_b->samplePos(time_b, &clipInfo_b->position_delta_interframe);
        ...
        node->setLocalPosition(aRibeiro::lerp(clipInfo_a->local_start, clipInfo_b->local_start, lrp));
    ...
    void RootMotionAnalyserData::applyMotionLocalExample() {
        if (!isClipsBlending()) {
            for (int i = 0; i < clip_count; i++) {
                ...
                _clipInfo.clip->root_node->setLocalPosition(aRibeiro::lerp(
            ...
            node->setLocalPosition(aRibeiro::lerp(
    ...
    void RootMotionAnalyserData::applyMotionGlobalExample() {
        if (!isClipsBlending()) {
            for (int i = 0; i < clip_count; i++) {
                ClipInfo &_clipInfo = clipInfo[i];
                _clipInfo.clip->root_node->setPosition(aRibeiro::lerp(
                    _clipInfo.start,
                    _clipInfo.position,
                    _clipInfo.weight
                ));
            ...
            node->setPosition(aRibeiro::lerp(
                clipInfo_a.position,
                clipInfo_b.position,
                clipInfo_b.weight
            ));
