...
int main(int argc, char* argv[]) {
    ...
    engine->initialSetup("Milky Way Studio", "Tech Demo: PBR", &CreateAppInstance);

    //                                  videoMode, fullscreen, sRGB, msaa_level
...
```cpp
mat4 = translate(Position) * toMat4(quat(Rotation)) * scale(Scale)
```
...
_1st_child_->LocalPosition = vec3(0, 10, 0);
_1st_child_->LocalRotation = quat(0, 0, 0, 1);
_1st_child_->LocalScale = vec3(1, 1, 1);
...
protected:
    ...
    void start();
    void OnMouseUp(sf::Mouse::Button button, aRibeiro::vec2 pos);
    void OnMouseDown(sf::Mouse::Button button, aRibeiro::vec2 pos);
